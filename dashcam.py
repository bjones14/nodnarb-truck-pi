...
def get_telemetry_raw():
    """Reads the latest JSON telemetry state from the RAM disk."""
    try:
...
def is_driving():
    """Returns True if the truck is awake (or if we are forcing the recording)."""
    if IGNORE_PARKED:
        return True
...
def ensure_paths():
    ...
    if not os.path.exists(RAM_DISK):
        ...
        # Clean up old stream files on start to prevent segment index errors
    ...
    if not os.path.exists(DISK_PATH):
        # We don't create this here because it should be handled by the mount/overlay logic
        pass
...
def get_telemetry():
    """Formats telemetry specifically for the SRT subtitles."""
    d = get_telemetry_raw()
